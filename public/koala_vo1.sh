...
# BASH AUTOCOMPLETION
# ----------------------------
echo "[+] Installing bash autocompletion for Koala..."
...
if ! grep -q "$COMPLETION_TARGET" "$USER_BASHRC"; then
  echo "source $COMPLETION_TARGET" >>"$USER_BASHRC"
  echo "[+] Added autocompletion to $USER_BASHRC"w
fi
